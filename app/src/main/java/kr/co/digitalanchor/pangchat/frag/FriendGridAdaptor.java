...
public class FriendGridAdaptor extends BaseAdapter {
    ...
    public View getView(final int index, View gridView, ViewGroup viewGroup) {
        ...
        //0 on 1 off 2 onAir
        // 옛날 것 1이 onAir
        if(mUsers.get(index).getIsOnAir() == 0){
            onAir.setVisibility(View.GONE);
        }else if(mUsers.get(index).getIsOnAir() == 1){
            onAir.setImageDrawable(getContext().getResources().getDrawable(R.drawable.icon_off));
            //onAir.setBackgroundResource(R.drawable.icon_off);
        }
